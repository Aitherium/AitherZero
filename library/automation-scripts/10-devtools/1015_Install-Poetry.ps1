...
try {
    ...
    $shouldInstall = $false
    if ($config.Features.DevTools -and $config.Features.DevTools.Poetry) {
        $poetryConfig = $config.Features.DevTools.Poetry
        $shouldInstall = $poetryConfig.Enabled -eq $true
    }
    ...
    $existingPoetry = Get-Command poetry
