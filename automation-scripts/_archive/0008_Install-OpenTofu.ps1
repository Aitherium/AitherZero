...
try {
    ...
    $shouldInstall = $false
    if ($config.Features.Infrastructure -and $config.Features.Infrastructure.OpenTofu) {
        $tofuConfig = $config.Features.Infrastructure.OpenTofu
        $shouldInstall = $tofuConfig.Enabled -eq $true
    }
    ...
    $version = if ($config.Features.Infrastructure.OpenTofu.Version -and $config.Features.Infrastructure.OpenTofu.Version -ne 'latest') {
        $config.Features.Infrastructure.OpenTofu.Version
    } else {
    ...
    if ($IsWindows) {
        ...
        $tempDir = if ($config.Features.Infrastructure.Directories -and $config.Features.Infrastructure.Directories.Configuration.LocalPath) {
            [System.Environment]::ExpandEnvironmentVariables($config.Features.Infrastructure.Directories.Configuration.LocalPath)
        } else {
            $env:TEMP
        }
    ...
    if ($config.Features.Infrastructure.OpenTofu.Configuration.Initialize -eq $true) {
        Write-ScriptLog "Initializing OpenTofu working directory..."

        $workingDir = if ($config.Features.Infrastructure.OpenTofu.Configuration.WorkingDirectory) {
            $config.Features.Infrastructure.OpenTofu.Configuration.WorkingDirectory
        } else {
            './infrastructure'
        }
