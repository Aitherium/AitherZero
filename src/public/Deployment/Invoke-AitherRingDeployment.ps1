...
function Invoke-AitherRingPromotion {
    ...
    # ══════════════════════════════════════════════════════════════
    # POST-DEPLOY: Cloudflare SSL/TLS Hardening Enforcement
    # ══════════════════════════════════════════════════════════════
    $sslScript = Join-Path $PSScriptRoot "../../../library/automation-scripts/30-deploy/3042_Enforce-CloudflareSSL.ps1"
    if (Test-Path $sslScript) {
        Write-Host "`n  ┌─ Post-Deploy: Cloudflare SSL/TLS Enforcement" -ForegroundColor Cyan
        if (-not $DryRun) {
            try {
                $sslResult = & $sslScript -Action verify 2>&1
                $sslExitCode = $LASTEXITCODE
                if ($sslExitCode -ne 0) {
                    Write-Host "  │  Drift detected — enforcing..." -ForegroundColor Yellow
                    & $sslScript -Action enforce 2>&1 | Out-Null
                    $sslExitCode = $LASTEXITCODE
                }
                $sslPassed = $sslExitCode -eq 0
                Write-Host "  └─ $(if ($sslPassed) { '✓ SSL/TLS compliant' } else { '⚠ SSL/TLS enforcement failed (non-blocking)' })" -ForegroundColor $(if ($sslPassed) { 'Green' } else { 'Yellow' })
                $gates += @{ Name = "SSL/TLS Hardening"; Passed = $sslPassed; Detail = if ($sslPassed) { "Compliant" } else { "Enforcement failed" } }
            } catch {
                Write-Host "  └─ ⚠ SSL check skipped: $_" -ForegroundColor Yellow
                $gates += @{ Name = "SSL/TLS Hardening"; Passed = $true; Detail = "Skipped: $_" }
            }
        } else {
            Write-Host "  │  [DRY RUN] Would verify/enforce Cloudflare SSL settings" -ForegroundColor Yellow
            Write-Host "  └─ ✓ DRY RUN" -ForegroundColor Green
            $gates += @{ Name = "SSL/TLS Hardening"; Passed = $true; Detail = "DRY RUN" }
        }
    }
