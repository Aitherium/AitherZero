...
begin {
    ...
    if (-not (Get-Variable -Name DashboardConfig -Scope Script -ErrorAction SilentlyContinue)) {
        $script:DashboardConfig = @{}
    }
    if (-not (Get-Variable -Name CollectedMetrics -Scope Script -ErrorAction SilentlyContinue)) {
