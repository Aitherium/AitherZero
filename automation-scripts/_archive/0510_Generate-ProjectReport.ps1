...
.NOTES
    Stage: Reporting
    Order: 0510
    Tags: reporting, documentation, status
#>

[CmdletBinding(SupportsShouldProcess)]
param(
    [string]$ProjectPath = ($PSScriptRoot | Split-Path -Parent | Split-Path -Parent),
    [string]$OutputPath = (Join-Path ($PSScriptRoot | Split-Path -Parent | Split-Path -Parent) "AitherZero/library/tests/reports"),
...
function Write-ReportLog {
    ...
    if (Get-Command Write-AitherLog -ErrorAction SilentlyContinue) {
        Write-AitherLog -Level $Level -Message $Message -Source "ProjectReport"
    } else {
