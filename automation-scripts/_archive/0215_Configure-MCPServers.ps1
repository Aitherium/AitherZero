...
if (-not $projectRoot) {
    Write-Warning "AitherZero module not found at expected location."
}

# Define Write-ScriptLog if not available (fallback)
...
# Define Write-ColorOutput if not available (fallback)
...
try {
    ...
    if ($workspaceRoot -and -not (Test-Path (Join-Path $workspaceRoot "AitherZero/library/integrations"))) {
        Write-ColorOutput "AITHERZERO_ROOT ($workspaceRoot) appears incorrect (missing AitherZero/library/integrations). Recalculating." -Level 'Warning'
        $workspaceRoot = $null
    }
    ...
    try {
        ...
        if ($nodeVersion) {
            ...
            $nodeMajor = [int]($nodeVersion -replace 'v(\d+)\..*', '$PSScriptRoot')
            if ($nodeMajor -lt 18) {
    ...
    $servers = [PSCustomObject]@{
        'aitherzero'          = [PSCustomObject]@{
            ...
            command = "node"
            args    = @(
                "`${workspaceFolder}/library/integrations/mcp-server/scripts/start-with-build.mjs"
            )
            env     = [PSCustomObject]@{
        ...
        # Git server removed as it is currently problematic/non-existent in registry
        # 'git' = [PSCustomObject]@{ ... }
