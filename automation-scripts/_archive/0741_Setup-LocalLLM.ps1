...
<#
.SYNOPSIS
    Installs and configures local LLM models for AitherOS services.
.DESCRIPTION
    Sets up Ollama with local models for AitherOS services. Supports:
    - Aither Tier Models (Nemotron architecture)
    - General purpose models (Mistral, LLaMA, Qwen, etc.)
    - Vision models for AitherVision
    
    The script ensures Ollama is running, pulls the requested model,
    and optionally pulls vision models for AitherVision.
.PARAMETER Model
    The local model to use. Categories:

    === AITHER TIER MODELS (Nemotron Architecture) ===
    - aither-reflex    (Tier 0) - Nemotron-Nano-9B-v2 - Fast responses, neurons
    - aither-router    (Tier 1) - Nemotron-Orchestrator-8B - Meta-routing
    - aither-agent     (Tier 2) - Nemotron-3-Nano-30B-A3B-FP8 - Tool calling
    - aither-reasoning (Tier 3) - Nemotron-3-Nano-30B-A3B - Deep thinking
    ...
    Default: llama3.2
.PARAMETER BuildAitherTiers
    Build all 4 Aither tier models using Build-AitherModels.ps1.
    This creates the full Nemotron-based model architecture.
.PARAMETER SkipVision
    Skip pulling vision models (llama3.2-vision) for AitherVision.
.EXAMPLE
    # Pull default model (llama3.2) for quick setup
    .\0741_Setup-LocalLLM.ps1
.EXAMPLE
    # Build all Aither tier models
    .\0741_Setup-LocalLLM.ps1 -BuildAitherTiers
.EXAMPLE
    # Pull specific balanced model
    .\0741_Setup-LocalLLM.ps1 -Model mistral-nemo
.EXAMPLE
    # Pull tier 0 model for fast responses
    .\0741_Setup-LocalLLM.ps1 -Model aither-reflex
.NOTES
    Stage: AI Tools
    Order: 0741
#>
...
param(
    [ValidateSet(
        # === AITHER TIER MODELS (Nemotron Architecture) ===
        # Tier 0 - Fast/Reflex
        "aither-reflex",           # Nemotron-Nano-9B-v2 - Fast tier-0, neurons
        # Tier 1 - Router
        "aither-router",           # Nemotron-Orchestrator-8B - Meta-routing
        # Tier 2 - Agent  
        "aither-agent",            # Nemotron-3-Nano-30B-A3B-FP8 - Tool calling
        # Tier 3 - Reasoning
        "aither-reasoning",        # Nemotron-3-Nano-30B-A3B - Deep thinking
...
Ensure-FeatureEnabled -Section "Features" -Key "AI.Ollama" -Name "Ollama"

Write-Host "🤖 Setting up local LLM models for AitherOS..." -ForegroundColor Cyan
Write-Host ""
...
switch ($Model) {
    # Aither Tier Models (Nemotron Architecture)
    "aither-reflex" {
        Write-Host "   Category: Aither Tier 0 (Fast/Reflex)"
        Write-Host "   Name: Nemotron-Nano-9B-v2"
        Write-Host "   VRAM Required: ~6GB"
        Write-Host "   Speed: Very Fast"
        Write-Host "   Best For: Fast tier-0 responses, neuron workers, speculative decoding"
    }
    "aither-router" {
        Write-Host "   Category: Aither Tier 1 (Router)"
        Write-Host "   Name: Nemotron-Orchestrator-8B"
        Write-Host "   VRAM Required: ~8GB"
        Write-Host "   Speed: Fast"
        Write-Host "   Best For: Meta-routing to specialist models, task classification"
    }
    "aither-agent" {
        Write-Host "   Category: Aither Tier 2 (Agent)"
        Write-Host "   Name: Nemotron-3-Nano-30B-A3B-FP8"
        Write-Host "   VRAM Required: ~16GB (MoE: 30B total, 3.6B active)"
        Write-Host "   Speed: Medium"
        Write-Host "   Best For: Multi-step tool calling, complex agentic tasks"
    }
    "aither-reasoning" {
        Write-Host "   Category: Aither Tier 3 (Reasoning)"
        Write-Host "   Name: Nemotron-3-Nano-30B-A3B (Full Precision)"
        Write-Host "   VRAM Required: ~40GB"
        Write-Host "   Speed: Slow"
        Write-Host "   Best For: Deep SASE reasoning, extended thinking, maximum quality"
    }
...
# Handle Aither tier model aliases - these need Build-AitherModels.ps1
$aitherTierModels = @{
    "aither-reflex" = "nvidia/nemotron-nano-9b-v2:latest"      # Tier 0
    "aither-router" = "nvidia/nemotron-orchestrator-8b:latest" # Tier 1
    "aither-agent" = $null   # Requires GGUF build
    "aither-reasoning" = $null # Requires GGUF build
}

if ($BuildAitherTiers) {
    Write-Host ""
    Write-Host "🏗️  Building Aither Tier Models..." -ForegroundColor Cyan
    Write-Host "   This builds all 4 Nemotron-based tier models for AitherOS."
    Write-Host ""
    
    $buildScript = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "AitherOS/scripts/Build-AitherModels.ps1"
    if (Test-Path $buildScript) {
        & $buildScript -Tier all -Pull
    } else {
        Write-Warning "⚠️  Build-AitherModels.ps1 not found at: $buildScript"
        Write-Host "   You can run it manually: AitherOS/scripts/Build-AitherModels.ps1 -Tier all -Pull"
    }
} elseif ($aitherTierModels.ContainsKey($Model)) {
    $ollama_alias = $aitherTierModels[$Model]
    if ($null -eq $ollama_alias) {
        Write-Warning "⚠️  $Model requires GGUF build. Running Build-AitherModels.ps1..."
        $tier = $Model -replace "aither-", ""
        $buildScript = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "AitherOS/scripts/Build-AitherModels.ps1"
        if (Test-Path $buildScript) {
            & $buildScript -Tier $tier -Pull
        } else {
            Write-Error "❌ Cannot build $Model - Build-AitherModels.ps1 not found"
            exit 1
        }
    } else {
        $modelsToPull += $ollama_alias
    }
} else {
    $modelsToPull += $Model
}
...
# 5. Pull vision model for AitherVision (unless skipped)
if (-not $SkipVision) {
    Write-Host ""
    Write-Host "👁️  Ensuring vision model for AitherVision..." -ForegroundColor Cyan
    ...
    if ($modelList -match "llama3.2-vision") {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Warning "⚠️  Failed to pull vision model - AitherVision may use fallback"
        }
    }
}
...
Write-Host ""
Write-Host "🚀 AitherOS Services can now use local LLM inference:" -ForegroundColor Cyan
Write-Host "   - AitherMind (port 8088) - Core thinking"
Write-Host "   - AitherVision (port 8084) - Image analysis"
Write-Host "   - AitherLLM (port 8118) - Unified gateway"
Write-Host ""
Write-Host "🏗️  To build all 4 Aither tier models (Nemotron architecture):" -ForegroundColor Cyan
Write-Host "   pwsh -File ./AitherOS/scripts/Build-AitherModels.ps1 -Tier all -Pull"
Write-Host ""
