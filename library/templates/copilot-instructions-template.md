...
**Critical**: For new installations, run `./bootstrap.ps1` to set up the environment. For existing installations, the module is auto-loaded via `Import-Module` or `bootstrap.ps1 -Mode New -Playbook genesis-bootstrap`.
...
Invoke-AitherPlaybook -Name test-quick # Fast validation
Invoke-AitherPlaybook -Name test-full # Complete tests
...
/home/runner/work/AitherZero/AitherZero/
 ...
 bootstrap.ps1 -Mode New -Playbook genesis-bootstrap # Main entry point - interactive mode
...
**Main Entry** (`bootstrap.ps1 -Mode New -Playbook genesis-bootstrap` - 500+ lines):
...
1. [X] Skipping bootstrap → [OK] Always run `./bootstrap.ps1 -Mode Update` first
2. [X] Running `az 0407` → [OK] Use `./az.ps1 0407` (needs ./ prefix)
3. [X] Forgetting Export-ModuleMember → [OK] Add function to export list
4. [X] Adding function without tests → [OK] Create matching `*.Tests.ps1` file
5. [X] Hardcoding values → [OK] Use `config.psd1` or `Get-Configuration`
6. [X] Using Write-Host → [OK] Use `Write-CustomLog` or `Write-UIText`
7. [X] Breaking cross-platform → [OK] Check `$IsWindows`, `$IsLinux`, `$IsMacOS`
8. [X] Stopping on known failures → [OK] Verify failures match expected list
