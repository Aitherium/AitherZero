...
try {
    ...
    $validationConfig = if ($config.Features.Infrastructure -and $config.Features.Infrastructure.ValidationTools) { $config.Features.Infrastructure.ValidationTools } else { @{} }

    if ($validationConfig.Enabled -eq $false) {
        Write-ScriptLog "Validation tools installation is disabled."
        exit 0
    }
    ...
    foreach ($toolInfo in $tools) {
        ...
        try {
            ...
            $installed = Get-Module -ListAvailable -Name $toolName
            ...
            if ($installed) {
                ...
                if ($validationConfig.CheckForUpdates -eq $true) {
                    ...
                    $onlineModule = Find-Module -Name $toolName
                ...
                # Verify installation
                $verifyInstall = Get-Module -ListAvailable -Name $toolName
                if ($verifyInstall) {
