...
- name: Run AitherZero
 ...
 ./bootstrap.ps1 -Mode New -Playbook genesis-bootstrap # Runs non-interactively
```
...
2. **Customize via parameters**:
 ...
 ./bootstrap.ps1 -Mode New -Playbook genesis-bootstrap -Profile Full
 ```
...
|------|---------|--------|-------------|
| `config.example.psd1` | Documented template | [OK] Yes | Reference/starting point |
| `config.psd1` | Main configuration | [OK] Yes | Shared team settings |
| `config.local.psd1` | Local overrides | [X] No | Personal preferences |
| `.env` | Environment variables | [X] No | Secrets/credentials |
...
jobs:
 ...
 - name: Run Tests
 run: Invoke-AitherPlaybook -Name test-ci
...
./bootstrap.ps1
Invoke-AitherPlaybook -Name test-ci
```
