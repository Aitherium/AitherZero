...
param(
    [string]$Path = (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent),
    [string]$OutputPath,
...
try {
        ...
        try {
            ...
            if ($isCI) {
                    ...
                    if ($Files.Count -gt 100) {
                        ...
                        for ($i = 0; $i -lt $totalFiles; $i += $batchSize) {
                            ...
                            foreach ($file in $batch) {
                                ...
                                try {
                                    $fileResult = Invoke-ScriptAnalyzer @fileParams
                                    if ($fileResult) {
                ...
                if ($analyzerParams.Path -is [array] -and $analyzerParams.Path.Count -gt 1) {
                    # Process files individually since Path only accepts string
                    $results = @()
                    foreach ($p in $analyzerParams.Path) {
                        $pParams = @{
                            Path = $p
                            Recurse = $false
                        }
                        if ($analyzerParams.ContainsKey('Severity')) { $pParams['Severity'] = $analyzerParams['Severity'] }
                        if ($analyzerParams.ContainsKey('ExcludeRule')) { $pParams['ExcludeRule'] = $analyzerParams['ExcludeRule'] }
                        if ($analyzerParams.ContainsKey('IncludeRule')) { $pParams['IncludeRule'] = $analyzerParams['IncludeRule'] }
                        if ($analyzerParams.ContainsKey('Settings')) { $pParams['Settings'] = $analyzerParams['Settings'] }
                        if ($analyzerParams.ContainsKey('Fix')) { $pParams['Fix'] = $analyzerParams['Fix'] }
                        if ($analyzerParams.ContainsKey('IncludeSuppressed')) { $pParams['IncludeSuppressed'] = $analyzerParams['IncludeSuppressed'] }
                        
                        $results += @(Invoke-ScriptAnalyzer @pParams)
                    }
                } else {
    ...
    Write-ScriptLog -Message "PSScriptAnalyzer analysis completed"
    ...
    if ($totalIssues -gt 0) {
        ...
        if (Get-Command -Name ConvertTo-SarifReport) {
    ...
    Write-Output "PSScriptAnalyzer analysis failed: $_"
