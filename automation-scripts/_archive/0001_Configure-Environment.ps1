...
# Provide stub functions if modules are not available (graceful degradation)
if (-not (Get-Command 'Get-EnvironmentConfiguration' -ErrorAction SilentlyContinue)) {
    function Get-EnvironmentConfiguration {
        param([string]$Category)
        Write-ScriptLog "Get-EnvironmentConfiguration not available - skipping configuration retrieval" -Level Warning
        return $null
    }
}

if (-not (Get-Command 'Set-EnvironmentConfiguration' -ErrorAction SilentlyContinue)) {
    function Set-EnvironmentConfiguration {
        param([string]$Category, [switch]$DryRun, [switch]$Force)
        Write-ScriptLog "Set-EnvironmentConfiguration not available - skipping configuration application" -Level Warning
        return @{ Success = $true; DryRun = $DryRun; AppliedChanges = @() }
    }
}

if (-not (Get-Command 'New-DeploymentArtifacts' -ErrorAction SilentlyContinue)) {
    function New-DeploymentArtifacts {
        param([string]$Platform, [string]$ConfigPath, [string]$OutputPath)
        Write-ScriptLog "New-DeploymentArtifacts not available - skipping artifact generation" -Level Warning
        return @{}
    }
}
