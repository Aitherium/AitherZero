...
try {
    ...
    if ($config.Features.Infrastructure -and $config.Features.Infrastructure.WSL2) {
        $wslConfig = $config.Features.Infrastructure.WSL2
        $shouldInstall = $wslConfig.Enabled -eq $true
    }
...
"@
        ...
        if ($null -ne $wslConfig.Settings.LocalhostForwarding) {
