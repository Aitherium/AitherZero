...
if (Test-Path $ScriptUtilPath) {
    Import-Module $ScriptUtilPath -Force
}
...
try {
    ...
    if (-not (Get-Command Initialize-InfrastructureSubmodule)) {
        Write-ScriptLog -Message "Infrastructure module not loaded, attempting to load..." -Level 'Warning'
        
        try {
            Import-Module (Join-Path $ProjectRoot "AitherZero/AitherZero/AitherZero.psd1") -Force -ErrorAction Stop
    ...
    try {
        if (Get-Command Get-Configuration) {
