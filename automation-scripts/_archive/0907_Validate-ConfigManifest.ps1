...
if (Test-Path './library/automation-scripts') {
    ...
    $uniqueNumbers = $allScripts | ForEach-Object { [int]($_.Name -replace '(\d+)_.*', '$PSScriptRoot') } | Sort-Object -Unique
...
$pathsToValidate = @(
    @{ Path = 'AitherZero/library/automation-scripts'; Type = 'Scripts' }
    @{ Path = 'tests/unit'; Type = 'Unit Tests' }
    @{ Path = 'tests/integration'; Type = 'Integration Tests' }
    @{ Path = 'AitherZero/library/playbooks'; Type = 'Playbooks' }
