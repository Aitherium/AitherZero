...
try {
    ...
    $systemConfig = if ($config.Features.Infrastructure -and $config.Features.Infrastructure.System) { $config.Features.Infrastructure.System } else { @{} }

    if ($systemConfig.Enabled -eq $false) {
        Write-ScriptLog "System configuration is disabled."
        exit 0
    }
