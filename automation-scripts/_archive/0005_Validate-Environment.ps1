...
function Write-ScriptLog {
    ...
    if (Get-Command Write-AitherLog -ErrorAction SilentlyContinue) {
        Write-AitherLog -Message $Message -Level $Level -Source "ValidateEnvironment"
    } else {
...
try {
    ...
    try {
        ...
        if ($config.Features.Core.Git.Required -eq $true) {
    ...
    try {
        ...
        if ($config.Features.Infrastructure.OpenTofu.Enabled -eq $true) {
    ...
    if ($IsWindows) {
        ...
        if ($config.Features.Infrastructure -and $config.Features.Infrastructure.HyperV -and $config.Features.Infrastructure.HyperV.Enabled -eq $true) {
            $hypervRequired = $true
        }
    ...
    $nodeRequired = $false
    if ($config.Features.Development -and $config.Features.Development.Node -and $config.Features.Development.Node.Enabled -eq $true) {
        $nodeRequired = $true
    }
    ...
    if ($config.Features.Development -and $config.Features.Development.Docker -and $config.Features.Development.Docker.Enabled -eq $true) {
        $dockerRequired = $true
    }
