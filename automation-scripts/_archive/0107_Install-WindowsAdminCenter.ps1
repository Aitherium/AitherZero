...
try {
    ...
    if ($config.Features.Infrastructure -and $config.Features.Infrastructure.WindowsAdminCenter) {
        $wacConfig = $config.Features.Infrastructure.WindowsAdminCenter
        $shouldInstall = $wacConfig.Enabled -eq $true
    }
    ...
    # Check registry
    $wacRegistry = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\ServerManagementGateway"
    if ($wacRegistry) {
        $wacInstalled = $true
    ...
    # Check service
    $wacService = Get-Service -Name ServerManagementGateway
    if ($wacService) {
        $wacInstalled = $true
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri $downloadUrl -MaximumRedirection 0
