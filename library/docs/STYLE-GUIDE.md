...
**Functions:**
```powershell
# [OK] CORRECT - Approved verb, PascalCase noun
Get-ConfigurationData
Set-ExtensionMode
Invoke-CommandParser

# [X] WRONG - Unapproved verb, wrong case
...
**Variables:**
```powershell
# [OK] CORRECT - PascalCase for important variables
$ConfigPath = "./config.psd1"
$ExtensionName = "MyExtension"

# [OK] CORRECT - camelCase for local/temporary variables
...
**Parameters:**
```powershell
# [OK] CORRECT - PascalCase with full names
param(
 [Parameter(Mandatory)]
...
```powershell
# [OK] CORRECT - Proper try/catch with meaningful errors
try {
...
# [X] WRONG - Empty catch or swallowing errors
try {
...
```powershell
# [OK] CORRECT - Use Write-CustomLog if available
...
```powershell
# [OK] CORRECT - Check platform variables
...
```powershell
# [OK] CORRECT - Use scriptblock evaluation for config.psd1
$configPath = "./config.psd1"
...
# [X] WRONG - Import-PowerShellDataFile fails with config.psd1
...
4. **SupportedModes must be consistent** with Invoke-AitherScript 5. **Extensions must exist** if listed in EnabledExtensions
...
```
./bootstrap.ps1 -Mode New -Playbook genesis-bootstrap -Mode <Mode> -Target <Target> [-Action <Action>] [-Options <Options>]
```
...
```
 [1] [TARGET] Mode Name - Description
