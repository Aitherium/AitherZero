...
try {
    ...
    if ($config.Features -and $config.Features.AITools -and $config.Features.AITools.GeminiCLI) {
        $geminiConfig = $config.Features.AITools.GeminiCLI
        $shouldInstall = $geminiConfig.Enabled -eq $true
    }
    ...
    if ($PSCmdlet.ShouldProcess('gemini-cli', 'Install globally via npm')) {
        try {
            ...
            $packageName = if ($geminiConfig.Version) {
                "@google/gemini-cli@$($geminiConfig.Version)"
            } else {
