...
try {
    ...
    if ($config.Features.Infrastructure -and $config.Features.Infrastructure.Go) {
        $goConfig = $config.Features.Infrastructure.Go + $goConfig  # Merge with defaults
        $shouldInstall = $goConfig.Enabled -eq $true
    }
    ...
    # Try Core Package Manager installation
    Write-ScriptLog "Using AitherZero Core Integration for Go installation"

    try {
        $preferredPackageManager = $goConfig.PreferredPackageManager
        $installResult = Install-AitherPackage -SoftwareName 'golang' -PreferredPackageManager $preferredPackageManager

        if ($installResult.Success) {
            Write-ScriptLog "Go installed successfully via $($installResult.PackageManager)"

            # Verify installation
            $version = Get-AitherSoftwareVersion -SoftwareName 'golang'
