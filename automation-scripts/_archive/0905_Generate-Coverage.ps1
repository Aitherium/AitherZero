...
# Set default paths
if (-not $SourcePath) {
    $SourcePath = Join-Path $projectRoot "AitherZero"
}
if (-not $TestPath) {
    $TestPath = Join-Path $projectRoot "AitherZero/library/tests"
}
if (-not $OutputPath) {
    $OutputPath = Join-Path $projectRoot "AitherZero/library/reports"
}
...
Write-ScriptLog "🔍 Starting code coverage analysis" -Source "0905_Generate-Coverage"
Write-ScriptLog "  Source: $SourcePath" -Source "0905_Generate-Coverage"
Write-ScriptLog "  Tests: $TestPath" -Source "0905_Generate-Coverage"
Write-ScriptLog "  Output: $OutputPath" -Source "0905_Generate-Coverage"
...
try {
    ...
    if (-not $pesterModule) {
        Write-ScriptLog "❌ Pester 5.0.0+ is required. Installing..." -Level Warning -Source "0905_Generate-Coverage"
    ...
    if ($IncludeBootstrap) {
        $startScript = Join-Path $projectRoot "bootstrap.ps1 -Mode New -Playbook genesis-bootstrap"
