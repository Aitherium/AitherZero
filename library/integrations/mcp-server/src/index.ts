...
/**
 * Infrastructure Deploy — full IaC lifecycle via OpenTofu + Genesis approval workflow.
 * Supports Docker, AWS, Azure, GCP, Hyper-V providers with plan/apply/destroy/approve/reject.
 */
async function infrastructureDeploy(action: string, args: Record<string, any>): Promise<string> {
  const genesisUrl = process.env.AITHER_GENESIS_URL || 'http://localhost:8001';

  switch (action) {
    case 'request': {
      // Submit a new infrastructure request via Genesis
      const body: Record<string, any> = {
        name: args.name || 'unnamed-deployment',
        provider: args.provider || 'docker',
        environment: args.environment || 'dev',
        profile: args.profile || 'minimal',
      ...
      if (args.auto_destroy_hours) body.auto_destroy_hours = args.auto_destroy_hours;

      try {
        const resp = await fetch(`${genesisUrl}/infra/requests`, {
          method: 'POST',
        ...
        return `Failed to submit request to Genesis: ${err}`;
      }
    }

    case 'plan': {
      // Dry-run plan via Genesis
      const body = {
        provider: args.provider || 'docker',
        environment: args.environment || 'dev',
        profile: args.profile || 'minimal',
      ...
      if (args.region) (body as any).region = args.region;

      try {
        const resp = await fetch(`${genesisUrl}/infra/plan`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        });
        ...
        const psArgs = [`-Action 'Plan'`];
        if (args.provider) psArgs.push(`-Provider '${args.provider}'`);
        if (args.environment) psArgs.push(`-Environment '${args.environment}'`);
        psArgs.push('-PassThru');

        const command = `
          cd '${AITHERZERO_ROOT}'
          Import-Module ./AitherZero/AitherZero.psd1 -Force -ErrorAction SilentlyContinue
          Invoke-AitherInfra ${psArgs.join(' ')} | ConvertTo-Json -Depth 5
        `;
        const { stdout, stderr } = await executePowerShell(command);
        return stderr ? `${stdout}\n\nErrors: ${stderr}` : stdout;
      }
    ...
    case 'apply': {
      // Apply via PowerShell (for approved requests)
      const psArgs = [`-Action 'Apply'`];
      if (args.request_id) psArgs.push(`-RequestId '${args.request_id}'`);
      if (args.provider) psArgs.push(`-Provider '${args.provider}'`);
      if (args.environment) psArgs.push(`-Environment '${args.environment}'`);
      psArgs.push('-AutoApprove');
      psArgs.push('-PassThru');
    ...
    case 'destroy': {
      if (!args.request_id) return 'Error: request_id is required for destroy action';
      try {
        const resp = await fetch(`${genesisUrl}/infra/requests/${args.request_id}/destroy`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        });
        const data = await resp.json();
    ...
    case 'status': {
      if (args.request_id) {
        try {
          const resp = await fetch(`${genesisUrl}/infra/requests/${args.request_id}/status`);
          const data = await resp.json();
          return JSON.stringify(data, null, 2);
        } catch (err) {
          return `Failed to get status: ${err}`;
        }
      }
      // Fallback to PowerShell status check
      const command = `
        cd '${AITHERZERO_ROOT}'
        Import-Module ./AitherZero/AitherZero.psd1 -Force -ErrorAction SilentlyContinue
        Invoke-AitherInfra -Action 'Status' -PassThru | ConvertTo-Json -Depth 5
      `;
      const { stdout, stderr } = await executePowerShell(command);
      return stderr ? `${stdout}\n\nErrors: ${stderr}` : stdout;
    }
    ...
    case 'list': {
      try {
        ...
        if (args.provider) params.append('provider', args.provider);
        const resp = await fetch(`${genesisUrl}/infra/requests?${params.toString()}`);
        const data = await resp.json();
        return JSON.stringify(data, null, 2);
      } catch (err) {
        ...
        const command = `
          cd '${AITHERZERO_ROOT}'
          Import-Module ./AitherZero/AitherZero.psd1 -Force -ErrorAction SilentlyContinue
          Invoke-AitherInfra -Action 'List' -PassThru | ConvertTo-Json -Depth 5
        `;
        const { stdout, stderr } = await executePowerShell(command);
        return stderr ? `${stdout}\n\nErrors: ${stderr}` : stdout;
      }
    ...
    case 'approve': {
      if (!args.request_id) return 'Error: request_id is required for approve action';
      try {
        const resp = await fetch(`${genesisUrl}/infra/requests/${args.request_id}/approve`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        return `Failed to approve: ${err}`;
      }
    }

    case 'reject': {
      if (!args.request_id) return 'Error: request_id is required for reject action';
      try {
        const resp = await fetch(`${genesisUrl}/infra/requests/${args.request_id}/reject`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ comment: args.comment || '' }),
        });
        ...
        return `Failed to reject: ${err}`;
      }
    }

    case 'providers': {
      try {
        const resp = await fetch(`${genesisUrl}/infra/providers`);
        const data = await resp.json();
        return JSON.stringify(data, null, 2);
      } catch (err) {
        return `Failed to list providers: ${err}`;
      }
    }

    case 'templates': {
      try {
        const resp = await fetch(`${genesisUrl}/infra/templates`);
        const data = await resp.json();
        return JSON.stringify(data, null, 2);
      } catch (err) {
        return `Failed to list templates: ${err}`;
      }
    }
    ...
    case 'validate': {
      try {
        ...
        const resp = await fetch(`${genesisUrl}/infra/validate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        });
    ...
    default:
      return `Unknown infrastructure_deploy action: ${action}. Available: request, plan, apply, destroy, status, list, approve, reject, providers, templates, validate`;
  }
}
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: 'repo_sync',
      description: 'Synchronize changes across all repositories. Push to origin (AitherOS private), sync AitherZero subtree to public repo, and optionally trigger aither sync. This is the primary tool for agents to commit and push updates.',
      inputSchema: {
        type: 'object',
        properties: {
          action: {
    ...
    },
    {
      ...
      description: 'Deploy infrastructure using OpenTofu — containers to Docker hosts, cloud resources to AWS/Azure/GCP, or VMs to Hyper-V. Supports approval workflows for staging/prod. Actions: request (submit for approval), plan (dry-run preview), apply (deploy), destroy (teardown), status (check), list (all deployments), approve/reject (admin), providers (available modules), templates (pre-built configs).',
      inputSchema: {
        type: 'object',
        properties: {
          action: {
            type: 'string',
            enum: ['request', 'plan', 'apply', 'destroy', 'status', 'list', 'approve', 'reject', 'providers', 'templates', 'validate'],
            description: 'Operation: "request" submits for approval, "plan" dry-runs, "apply" deploys, "destroy" tears down, "status" checks, "list" shows all, "approve"/"reject" for admins, "providers" lists modules, "templates" lists presets.',
          },
          name: {
            type: 'string',
            description: '(request) Human-readable name for the deployment.',
          },
          provider: {
            type: 'string',
            enum: ['docker', 'aws', 'azure', 'gcp', 'hyperv', 'kubernetes'],
          ...
          profile: {
            type: 'string',
            enum: ['minimal', 'demo', 'full'],
            description: 'Sizing profile. Default: minimal.',
          },
          region: {
            type: 'string',
          ...
          services: {
            ...
            items: {
              ...
              properties: {
                ...
                ports: { type: 'array', items: { type: 'number' }, description: 'Ports to expose' },
                env: { type: 'object', description: 'Environment variables' },
                cpu: { type: 'number', description: 'CPU cores' },
                memory: { type: 'string', description: 'Memory (e.g. 2Gi)' },
                replicas: { type: 'number', description: 'Number of replicas' },
                health_path: { type: 'string', description: 'Health check path' },
                gpu_enabled: { type: 'boolean', description: 'Requires GPU' },
              },
              required: ['name', 'image'],
            },
          ...
          variables: {
            type: 'object',
            description: 'Additional OpenTofu variables.',
          },
          justification: {
            type: 'string',
            description: '(request) Why this infrastructure is needed (shown to approvers).',
          },
          auto_destroy_hours: {
            type: 'number',
            description: 'Auto-destroy after N hours (cost control).',
          },
          template: {
            type: 'string',
            description: 'Use a pre-built template: single-container, aitheros-core-aws, aitheros-core-azure, gpu-node-hyperv, dev-environment, full-stack-gcp.',
          },
          request_id: {
            type: 'string',
            description: '(status/apply/destroy/approve/reject) Infrastructure request ID.',
          },
          comment: {
            type: 'string',
            description: '(approve/reject) Approver comment.',
          },
        },
        required: ['action'],
      },
    ...
    switch (name) {
      ...
      */
      ...
      case 'infrastructure_deploy':
        result = await infrastructureDeploy(
          args.action as string,
          args as Record<string, any>
        );
        break;
