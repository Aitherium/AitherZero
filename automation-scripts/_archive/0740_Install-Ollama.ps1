...
if ([string]::IsNullOrWhiteSpace($Model)) {
    $configModel = Get-AitherConfigs -Section "Features" -Key "AI.Ollama.DefaultModel" -ErrorAction SilentlyContinue
    if ($configModel) {
        $Model = $configModel
    }
    else {
...
if ($Port -eq 0) {
    $configPort = Get-AitherConfigs -Section "Features" -Key "AI.Ollama.Port" -ErrorAction SilentlyContinue
    if ($configPort) {
        $Port = [int]$configPort
    }
    else {
