...
try {
    ...
    if (Test-Path $infraPath) {
        ...
        $tfFiles = Get-ChildItem -Path $infraPath -Filter "*.tf" -File
        if ($tfFiles) {
            $tofuDirectories += $infraPath
        }

        # Check subdirectories
        $subDirs = Get-ChildItem -Path $infraPath -Directory
        foreach ($dir in $subDirs) {
            $tfFiles = Get-ChildItem -Path $dir.FullName -Filter "*.tf" -File
            if ($tfFiles) {
    ...
    if ($legacyInfraPath -and (Test-Path $legacyInfraPath)) {
        ...
        if (Test-Path $legacyTofu) {
            $tfFiles = Get-ChildItem -Path $legacyTofu -Filter "*.tf" -File
            if ($tfFiles) {
