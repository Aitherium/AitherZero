...
try {
    ...
    if ($config.Features.DevTools -and $config.Features.DevTools.VSBuildTools) {
        $vsBuildConfig = $config.Features.DevTools.VSBuildTools
        $shouldInstall = $vsBuildConfig.Enabled -eq $true
    }
