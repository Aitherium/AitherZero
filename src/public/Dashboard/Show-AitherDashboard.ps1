...
function Show-AitherDashboard {
    ...
    begin {
        ...
        # Layout-aware payload base: in the monorepo the module sits under the
        # product-vault tree, standalone (the public checkout) IS ProjectPath.
        # Same probe Get-AitherConfigs uses; without it every panel below reads
        # a directory that never exists for public users.
        $zeroMarker = Join-Path $ProjectPath '.PRODUCTS/.AITHERZERO/config/config.psd1'
        $payloadRoot = if (Test-Path $zeroMarker) { Split-Path (Split-Path $zeroMarker -Parent) -Parent } else { $ProjectPath }
    }
    ...
    process {
        ...
        function Show-ProjectMetrics {
            ...
            # Get latest report
            $reportPath = Join-Path $payloadRoot "library/tests/reports"
        ...
        function Show-TestResults {
            ...
            $testResultsPath = Join-Path $payloadRoot "library/tests/results"
        ...
        function Show-RecentLogs {
            ...
            $logPath = $null
            $searchPaths = @(
                (Join-Path $payloadRoot "library/logs"),
        ...
        function Show-ModuleStatus {
            ...
            $domains = Get-ChildItem -Path (Join-Path $payloadRoot "src/public") -Directory -ErrorAction SilentlyContinue

            if (-not $domains) {
                Write-AitherLog -Level Error -Message "No domains found at $(Join-Path $payloadRoot "src/public")" -Source 'Show-AitherDashboard'
            }
