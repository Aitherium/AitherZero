...
param(
    [string]$Path = (Join-Path (Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent) "tests/integration"),
    [string]$OutputPath,
    [switch]$DryRun,
    [switch]$PassThru,
    [switch]$IncludeE2E
...
try {
    ...
    $mainModule = Join-Path $projectRoot "AitherZero" "AitherZero.psd1"
    if (-not (Test-Path $mainModule)) {
        # Fallback: try old location (for backward compatibility during migration)
        $mainModule = Join-Path $projectRoot "AitherZero.psd1"
    }

    # Never -Force over an already-loaded AitherZero. This script runs as
    ...
    if ((Test-Path $mainModule) -and -not (Get-Module -Name 'AitherZero')) {
        try {
            ...
            $domainModules = Get-ChildItem -Path (Join-Path $projectRoot "AitherZero") -Filter "*.psm1" -Recurse
