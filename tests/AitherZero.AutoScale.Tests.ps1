...
Describe "AitherAutoScale Python Agent" -Tag "Integration", "AutoScale", "Python" {
    ...
    Context "Configuration Entries" {
        ...
        It "agent_cards has autoscale entry" {
            # Check agent_cards/ directory first, fall back to monolithic file
            $cardsDir = Join-Path $projectRoot "AitherOS" "config" "agent_cards"
            $cardsFile = Join-Path $projectRoot "AitherOS" "config" "agent_cards.yaml"
            $content = ""
            if (Test-Path $cardsDir) {
                $content = Get-ChildItem -Path $cardsDir -Filter "*.yaml" | ForEach-Object { Get-Content $_.FullName -Raw } | Out-String
            }
