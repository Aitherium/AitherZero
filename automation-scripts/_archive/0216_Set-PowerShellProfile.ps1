...
try {
    ...
    if ($config.Features.Development -and $config.Features.Development.PowerShellProfile) {
        $psProfileConfig = $config.Features.Development.PowerShellProfile
...
# AitherZero aliases
Set-Alias -Name aither -Value (Join-Path `$env:AITHERZERO_ROOT 'bootstrap.ps1 -Mode New -Playbook genesis-bootstrap') -ErrorAction SilentlyContinue
Set-Alias -Name az0 -Value (Join-Path `$env:AITHERZERO_ROOT 'bootstrap.ps1 -Mode New -Playbook genesis-bootstrap') -ErrorAction SilentlyContinue
...
"@
    ...
    if ($config.Features.Development.PowerShellProfile.CustomContent) {
        $ProfileNameContent += @"

# Custom content from configuration
$($config.Features.Development.PowerShellProfile.CustomContent)
