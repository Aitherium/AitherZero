...
try {
    ...
    if ($config.Features.Infrastructure -and $config.Features.Infrastructure.CertificateAuthority) {
        $caConfig = $config.Features.Infrastructure.CertificateAuthority
        $shouldInstall = $caConfig.Enabled -eq $true
    }
