...
.EXAMPLE
    ./0517_Aggregate-AnalysisResults.ps1 -SourcePath ./AitherZero/library/reports -GenerateRecommendations
.NOTES
    Stage: Reporting
    Category: Analysis
    Order: 0512
...
[CmdletBinding()]
param(
    [string]$SourcePath = "AitherZero/library/reports",
    [string]$OutputPath = "AitherZero/library/reports/analysis-summary.json",
    [switch]$IncludeComparison,
    [switch]$GenerateRecommendations
)

Import-Module (Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent) -Force
