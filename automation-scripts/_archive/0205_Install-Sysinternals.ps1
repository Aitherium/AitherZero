...
try {
    ...
    if ($config.Features.DevTools -and $config.Features.DevTools.Sysinternals) {
        $sysinternalsConfig = $config.Features.DevTools.Sysinternals
        $shouldInstall = $sysinternalsConfig.Enabled -eq $true
    }
    ...
    if (Test-Path $installPath) {
        $existingFiles = Get-ChildItem -Path $installPath -Filter '*.exe'
    ...
    # Verify installation
    $installedTools = Get-ChildItem -Path $installPath -Filter '*.exe'
