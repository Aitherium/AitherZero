...
function New-RingStatusHTML {
    ...
    # Load CSS template
    $cssPath = "./library/templates/dashboard/ring-status-css.css"
    ...
    [void]$html.AppendLine('        <h1>🔄 Ring Status Dashboard</h1>')
    ...
    foreach ($ring in $Data.rings) {
        ...
        if ($ring.prs -and $ring.prs.Count -gt 0) {
            foreach ($pr in $ring.prs) {
                ...
                [void]$html.AppendLine("                    <span>$($pr.author)</span> •")
                [void]$html.AppendLine("                    <span>$($pr.createdAt)</span> •")
