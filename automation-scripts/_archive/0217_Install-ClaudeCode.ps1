...
try {
    ...
    if ($config.Features.AITools -and $config.Features.AITools.ClaudeCode) {
        $claudeConfig = $config.Features.AITools.ClaudeCode
        $shouldInstall = $claudeConfig.Enabled -eq $true
    }
