...
<#
.SYNOPSIS
    Configures Windows Defender exclusions for AitherZero development environment.
...
function Expand-ConfigPath {
    ...
    # Handle special AITHERZERO_ROOT if not set
    if ($expanded -match '\$env:AITHERZERO_ROOT' -or $expanded -eq '$env:AITHERZERO_ROOT') {
        $aitherzeroRoot = $env:AITHERZERO_ROOT
        if (-not $aitherzeroRoot) {
            # Try to detect workspace root
            $aitherzeroRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
            if (Test-Path (Join-Path $aitherzeroRoot 'AitherZero')) {
                $aitherzeroRoot = $aitherzeroRoot
            }
        }
        if ($aitherzeroRoot) {
            $expanded = $expanded -replace '\$env:AITHERZERO_ROOT', $aitherzeroRoot
        } else {
            return $null  # Skip if we can't resolve
        }
    }
