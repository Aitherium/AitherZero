...
watch('./src', { recursive: true }, (eventType, filename) => {
 console.log(`[NOTE] Changed: ${filename}`);
 rebuild();
});
...
async function testServer() {
 ...
 console.log('[OK] Server responding');
...
async function validateConfig() {
 console.log('[SEARCH] Validating configuration...\n');
 
 // Check package.json
 if (!existsSync('package.json')) {
 console.error('[X] package.json not found');
 process.exit(1);
 }
 ...
 console.error('[X] Missing MCP SDK dependency');
 process.exit(1);
 }
 console.log('[OK] MCP SDK dependency present');
 
 // Check TypeScript config
 if (!existsSync('tsconfig.json')) {
 console.error('[X] tsconfig.json not found');
 process.exit(1);
 }
 console.log('[OK] tsconfig.json exists');
 
 // Check source files
 if (!existsSync('src/index.ts')) {
 console.error('[X] src/index.ts not found');
 process.exit(1);
 }
 console.log('[OK] Source files present');
